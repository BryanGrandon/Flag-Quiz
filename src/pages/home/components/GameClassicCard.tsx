...
const GameClassicCard = ({ title, howToPlay, img, keyType, options }: ClassicGame) => {
  ...
  return (
    <article className='flex flex-col p-4 gap-4 border border-gray-500 shadow-gray-700/30 rounded-xl shadow-lg'>
          ...
          <>
            <section className=' space-y-2'>
              ...
              <section className='flex flex-row gap-2 '>
                {options.map((option) => (
                  <SelectButton key={option.id} text={option.text} checked={activeMode} value={option.value} onClick={() => setActiveMode(option.value)} />
                ))}
              </section>
            </section>
            <span className='h-px rounded-xl w-full bg-gray-400 my-2'></span>
      ...
      <Button text='Start Quiz' onClick={() => startClassicGame({ type: keyType, mode: activeMode })} moreClasses='bg-secondary hover:bg-secondary/80 text-black w-full' />
    </article>
  )
}
