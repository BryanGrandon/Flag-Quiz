...
import type { GameMode, ImageOfTheFlag } from '../utilities/interfaces/gameContext'
...
const GameCountryProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const getValue = (item: Country, type: GameMode) => {
    return type === KEY_GAMES.COUNTRY ? item.name.common : item.capital[0]
  }

  const selectWinner = (type: GameMode) => {
  ...
  const generateOption = (winner: string, type: GameMode) => {
    const limit = 4
  ...
  const startMultipleChoiceGame = (type: GameMode) => {
  ...
  const startWrittenGame = (type: GameMode) => {
    selectWinner(type)
  }

  const startGamesModes = {
    multipleChoice: (type: GameMode) => startMultipleChoiceGame(type),
    writing: (type: GameMode) => startWrittenGame(type),
  }
